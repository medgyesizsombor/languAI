...
public class ChatGPTService : BaseService, IChatGPTService
{
    ...
    public async Task<List<ExerciseViewModel>> ReceiveExercisesFromChatGPT(ExerciseRequestViewModel request, string existingLearningWords, string existingNativeWords)
    {
        ...
        for (int i = 0; i < 1; i++)
        {
            ...
            var randomWordIndex = random.Next(0, existingLearningWords.Length - 1);
            const string messageFromUser = "Generate one exercise please!";

            string messageFromSystem = exerciseType switch
            {
                ExerciseTypeEnum.MissingWordExercise => $"You are a language teacher who is creating a Missing Word exercise.\n" +
                                        $"Parameters:\n" +
                                        $"-Topic name: {request.TopicDescription},\n" +
                                        $"-Word: {existingLearningWords[randomWordIndex]},\n" +
                ...
                ExerciseTypeEnum.SentenceAssemblyExercise => $"You are a language teacher who is creating Sentence Assembly Exercise.\n" +
                                        $"Parameters:\n" +
                                        $"-Topic name: {request.TopicDescription},\n" +
                                        $"-Words: {existingLearningWords[randomWordIndex]},\n" +
                                        $"-Language: {currentLearning.LearningLanguageName},\n" +
                                        $"-Previous sentences: {previousMainSentences},\n" +
                                        $"-Language Level: beginner,\n\n" +
                                        $"Requirements:\n" +
                                        ...
                                        $"  ]\n" +
                                        $"}}\".",
                ExerciseTypeEnum.WordPairingExercise => $"You are a language teacher who is creating Sentence Assembly Exercise.\n" +
                                        $"Parameters:\n" +
                                        $"-Topic name: {request.TopicDescription},\n" +
                                        $"-Previous sentences: {previousMainSentences},\n" +
                                        $"-Language Level: beginner,\n\n" +
                                        ...
                                        $"  }}\n" +
                                        $"]}}",
                ExerciseTypeEnum.MistakeCorrectingExercise => $"You are a language teacher who is creating a mistake correcting exercise.\n" +
                                        $"Parameters:\n" +
                                        $"-Topic name: {request.TopicDescription},\n" +
                                        ...
                                        $"  ]" +
                                        $"}}\n",
                _ => $"You are a language teacher who is creating a question answering exercise.\n" +
                                        $"Parameters:\n" +
                                        $"-Topic name: {request.TopicDescription},\n" +
                                        $"-Existing Words: {existingLearningWords},\n" +
                                        $"-Language: {currentLearning.LearningLanguageName},\n" +
